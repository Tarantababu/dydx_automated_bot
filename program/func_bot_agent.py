...
class BotAgent:
    ...
    # Send z-scores of open positions
    async def send_open_positions_zscores(self):
        try:
            # Fetch recent candles for each market
            prices_m1 = await get_candles_recent(self.client, self.market_1)
            prices_m2 = await get_candles_recent(self.client, self.market_2)

            # Calculate spread
            spread = prices_m1 - (self.hedge_ratio * prices_m2)

            # Calculate z-score
            zscore = calculate_zscore(spread)

            # Send z-score
            message = f"Z-Score for {self.market_1} and {self.market_2}: {zscore.iloc[-1]}"
            send_message(message)

        except Exception as e:
            print(f"Error sending z-scores: {e}")
